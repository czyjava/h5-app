export type HomeAiPageRoute = 'home' | 'design' | 'discover' | 'mine' | 'api-debug';

export interface HomeAiHashRoute {
  page: HomeAiPageRoute;
  featureCode?: string;
  nativePath?: string;
}
...
export function parseHomeAiHashRoute(hash: string): HomeAiHashRoute {
  if (!hash || hash === '#') {
    return { page: 'home' };
  }
...
export function buildHomeAiHashRoute(route: HomeAiHashRoute) {
  switch (route.page) {
    case 'api-debug':
      return '#/api-debug';
    case 'discover':
