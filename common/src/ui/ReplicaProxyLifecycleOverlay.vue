...
const defaultEndpoints: ReplicaConsoleEndpoints = {
  snapshot: '/__replica_proxy_events/snapshot',
  stream: '/__replica_proxy_events/stream',
  clear: '/__replica_proxy_events/clear',
};
...
const props = withDefaults(
  defineProps<{
    ...
    title?: string;
    subtitle?: string;
    endpoints?: Partial<ReplicaConsoleEndpoints>;
    maxEvents?: number;
  }>(),
  {
    pageMode: false,
...
const endpoints = computed<ReplicaConsoleEndpoints>(() => ({
  ...defaultEndpoints,
  ...props.endpoints,
}));
const debugPageUrl = computed(() => `${window.location.origin}${window.location.pathname}${window.location.search}${props.pageHash}`);
...
const filteredRequestGroups = computed(() => {
  ...
  // 调试控制台复用神笔 Network 的组合过滤：域名、状态和关键词逐层收窄。
...
function buildRawExchange(group: ProxyRequestGroup): RawExchange {
  ...
  // 只把开发者真正关心的浏览器请求、上游请求和上游响应重组出来，生命周期细节留在代理日志里。
  return {
    request: {
      id: group.id,
      method: group.method,
...
async function loadSnapshot() {
  loading.value = true;
  try {
    const response = await fetch(endpoints.value.snapshot);
    const payload = (await response.json()) as { events?: ProxyLifecycleEvent[] };
    events.value = (payload.events ?? []).slice(-props.maxEvents);
    selectLatestIfNeeded();
  } catch (error) {
    connected.value = false;
    console.warn('[Replica Debug] 读取代理快照失败', error);
  } finally {
    loading.value = false;
  }
}
...
function connectEventStream() {
  ...
  // 通过 SSE 接收公共透明代理事件，页面里只展示整理后的 request / response。
  eventSource = new EventSource(endpoints.value.stream);
  eventSource.onopen = () => {
  ...
  eventSource.onmessage = (message) => {
    try {
      ...
      events.value = [...events.value, event].slice(-props.maxEvents);
      selectLatestIfNeeded();
    } catch (error) {
      console.warn('[Replica Debug] 代理事件解析失败', error);
    }
  };
}
...
async function clearEvents() {
  try {
    await fetch(endpoints.value.clear, { method: 'POST' });
  } finally {
    events.value = [];
    selectedId.value = '';
  }
...
.api-debug-launcher {
  position: fixed;
  right: var(--replica-debug-launcher-right, max(18px, calc((100vw - 430px) / 2 + 18px)));
  bottom: var(--replica-debug-launcher-bottom, 132px);
  left: var(--replica-debug-launcher-left, auto);
  z-index: 95;
  width: 46px;
  height: 46px;
  display: inline-grid;
